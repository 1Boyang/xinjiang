...
const createFormData = (formData) => {
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      path: `/v1/form/${appType}/${formUuid}/saveFormData`,
      method: 'POST',
      headers: {
      ...
      res.on('end', () => {
        try {
          ...
          if (result.success) {
            ...
            reject(new Error(result.errorMsg || result.throwable));
          }
        } catch (e) {
...
(async () => {
  for (const item of wldwData) {
    try {
      await createFormData(item);
      console.log('✓', item.textField_mnznbh90);
    } catch (e) {
      console.log('✗', item.textField_mnznbh90, e.message);
    }
  }
  console.log('往来单位完成');
})();